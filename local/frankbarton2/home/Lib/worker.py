...
def LIB_process_task():
	...
	local_tasks = [ path for path in schema._user('Tasks')['$FILES'] if path.endswith('.task') ]
	
	facility_tasks = [ path for path in schema.home('Tasks')['$FILES'] if path.endswith('.task') ]
	...
	if tasks:
		...
		brain.Task << sop.sh( brain.Task.file )
...
def LIB_build_worker():
	...
	brain.Worker = sop.Brain() << sop.sh( brain.Task.file.replace( '.task' , '.worker' )  )
...
def LIB_render_frame():
	...
	while 1:
		...
		if os.path.exists( worker_lock_file ):
			...
			brain.Worker = sop.Brain() << sop.sh( worker_file  )
			...
			while 1:
				...
				#brain.Worker >> sop.sh( worker_lock_file )
...
def start():
	...
	brain.Lib.database = sop.sh.Lib.database()
	brain.Lib.worker = medula.knife.Lib.worker()
	
	farm_db = brain.Lib.database.Connection( schema.home( 'Farm/Databases/tasks.db' )['file'] )
