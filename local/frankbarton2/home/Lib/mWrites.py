...
def process_category( this ):
	...
	Category << ( brain.Categories( Category.category , None , create_att = False ) or sop.Brain() )
...
def build_path( this , return_params = False ):
	...
	print 'DEBUG::::' , this.UNIT_PATH

	
	OUTPUT_PATH = sop.Normalize.join( this.UNIT_PATH , brain.Project.OUTPUT , this.VALUES.wm_category  )
	...
	OUTPUT_PATH = sop.Normalize.join( OUTPUT_PATH , folder , render_pattern  )

	PROXY_PATH = brain.Lib.sequence.proxy_res_path( OUTPUT_PATH ) if this.VALUES.wm_path_target == 'both' else ''
	
	if this.VALUES.wm_path_target == 'full':
