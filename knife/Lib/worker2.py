...
El proceso de tasking implica 2 pasos principales:

1. Desde un script salvado en una unidad definida en un RESOURCE 
	1.1 Selección de un Write, previamente configurado con un framerange
	1.2 Se selecciona el modo de render ( full or proxy ) , donde se va a realizar 
		la tarea ( local o farm ) y la forma de procesar los fotogramas ( linear , reversed, preview o random )
		
	1.3 Se lanza la tarea, al hacerlo se crea un archivo 
...
.task files are a regular sqlite database file generated selecting a Write node
and pushing create task.

The script is saved and copied into UNIT/nuke/_taskScripts , if the UNIT cannot be reached 
the task cannot be processed.

The .task file has two tables, TASK and FRAMES

TASK is a single-row table with info about current task

emmiter_host , script_host , script_resource , script_path , node_to_render , status , percent_completed
 
FRAMES has one row for frame to render
