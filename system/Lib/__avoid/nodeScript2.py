...
def knobs_by_tokenize( path , nspace ):
	...
	for line in brain.Lib.tokenize2.tokenize_classes( sop.sh( path )['read'] ):
...
def __execute_with_knobs( path ):
	...
	#nspace = sop.Core.execution( path , nspace )  #This function executes and set __called__
	
	#print '****' , nspace['info']
	
	#brain.nscript[ path ].space = nspace

	#print '3 DEBUG EOF sop.Core.execution' , nspace
