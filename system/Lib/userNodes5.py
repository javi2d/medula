...
def knobChanged():
	...
	if this.SAFE_NODE and 'userNode_id' in nuke.thisNode().knobs():

	#if this.KNOBS and this.KNOBS( 'userNode_id', None , create_att = False  ):
...
# the knobChanged callback for userNodes is autoincluded here

#brain.Lib.include.CALLBACK( 'userNode/knobChanged' , knobChanged )
	...
	for knob in [ k for k in nbrain.knobs if k not in '[ ]'.split() ]:
		...
		try:
			...
			if nbrain.knob_callbacks.get( knob.name() , None ) and hasattr( knob , 'setCommand' ):
				...
				knob.setCommand( 'brain.Lib.userNodes5.button_callback( "%s" , "%s" )' % ( nbrain.name , knob.name() )  )
